...
try:
    sock.connect(("192.168.108.35", 3000))
...
def fnReadDistance():
    ...
    g_f64Curr_x = 0.0
    g_f64Curr_y = 0.0
    if g_startSamplingDistflag == True:
        ...
        for sLine in sData.splitlines():
            sRawGprmcData = sLine.split(",")
            if sRawGprmcData[0] == "$GPRMC":
                sParseResult = pynmea2.parse(sLine)
                g_f64RawData[1], g_f64RawData[2] = sParseResult.latitude, sParseResult.longitude
        g_f64Curr_x, g_f64Curr_y, __temp__, __temp__ = utm.from_latlon(np.float64(g_f64RawData[1]), np.float64(g_f64RawData[2]))
        ...
        if g_f64Prev_X == 0.0 and g_f64Prev_y == 0.0:
            ...
            if(m_f64_Distance_UTM >= g_samplingDistance):
                ...
                Thread(target=fnGetDepth).start()
                g_GprmcBuffer.put(sParseResult.timestamp)
                g_GprmcBuffer.put(g_f64RawData[1])
                g_GprmcBuffer.put(g_f64RawData[2])
                g_GprmcBuffer.put(sParseResult.spd_over_grnd)
                g_startLoggingflag = True
                g_f64Prev_X = g_f64Curr_x
                g_f64Prev_y = g_f64Curr_y
    Thread(target=fnReadDistance).start()
...
def main():
    ...
    tThreadFour = Thread(target=fnReadDistance)
    tThreadOne.start()
    tThreadTwo.start()
    tThreadThree.start()
    tThreadFour.start()
